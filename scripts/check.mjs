...
for (const file of ['dist/app.js', 'dist/catalog.js', 'scripts/serve.mjs', 'server/multilingual.js', 'server/query-expansion.js', 'server/hosting.js', 'server/embeddings.js', 'server/remote.js', 'server/sources.js', 'server/asian-sources.js', 'server/search.js']) execFileSync(process.execPath, ['--check', fileURLToPath(new URL(file, project))], { stdio: 'inherit' });
